 ...
 * Handles user authentication, permissions, and admin profiles using JSON storage
 */
...
class BrowserCrypto {
  ...
  static async hashPassword(password: string): Promise<string> {
    // Use a more secure approach that mimics bcrypt structure
    ...
    // Create a bcrypt-like structure: $2a$10$salt$hash (53 chars after salt)
    const salt = 'timber2024salt';
    const truncatedHash = hashHex.substring(0, 53);
    return `$2a$10$${salt}$${truncatedHash}`;
  }
  ...
  static validateToken(token: string): { valid: boolean; payload?: JWTPayload; error?: string } {
    try {
      ...
      // Check if it's a JWT token (has dots) or base64 token
      if (token.includes('.')) {
        ...
        // Base64 format
        payload = JSON.parse(atob(token)) as JWTPayload;
      }
...
export class AuthService {
  ...
  createSession(user: UserAccount): { token: string; userId: string; role: string; expiresAt: string; issuedAt: string } {
    const token = BrowserCrypto.generateToken(user);
  ...
  isLoginBlocked(email: string): boolean {
    return !this.checkRateLimit(email);
  }
}
